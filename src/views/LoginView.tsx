...
const LoginView: React.FC<LoginViewProps> = ({ setUsers, setCurrentUser, users }) => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (isLogin) {
      ...
      if (users[formData.email]) { setError('An account with this email already exists.'); return; }
      if (!formData.name || !formData.companyName || !formData.email) { setError('Please fill out all fields.'); return; }
      const newUser: User = {
        id: `user-${Date.now()}`,
        name: formData.name,
  ...
  return (
    <div className="bg-gradient-to-b from-slate-900 to-slate-950 min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-800/60 backdrop-blur-lg p-8 rounded-2xl shadow-2xl border border-slate-700/50">
        ...
        <p className="text-center text-gray-400 mb-2 text-sm">EASY Agent Hub</p>
        <p className="text-center text-gray-500 mb-8 text-xs">{isLogin ? 'Login to access your AI agents' : 'Get started with T, A & The Boss'}</p>

        {error && <p className="bg-red-500/20 text-red-300 p-3 rounded-lg mb-6 text-center text-sm">{error}</p>}
