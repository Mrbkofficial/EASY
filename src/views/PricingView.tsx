...
const CheckIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-green-400 flex-shrink-0">
  ...
  return (
    <div className="p-4 md:p-6 text-white h-full pt-24 pb-20 overflow-y-auto">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-400 mt-2">Unlock agents, missions, and more power.</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
        {PRICING_PLANS.map(plan => (
          <div key={plan.tier} className={`bg-gray-800 rounded-xl p-6 flex flex-col border-2 transition-all duration-300 ${plan.recommended ? 'border-green-500' : 'border-gray-700'}`}>
            {plan.recommended && (
              <div className="text-center bg-green-500 text-white text-xs font-bold py-1 px-3 rounded-full -mt-9 mx-auto shadow-lg">RECOMMENDED</div>
            )}
            <h3 className="text-xl font-bold text-white text-center mt-2">{plan.tier}</h3>
            <p className="text-center text-gray-400 text-sm">{plan.requests}</p>
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg font-semibold transition-colors duration-200 ${user.tier === plan.tier ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700 text-white'}`}
            >
