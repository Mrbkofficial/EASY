...
const ProjectsView: React.FC<ProjectsViewProps> = ({ user, onUpdateUser }) => {
  ...
  const handleEditResult = (result: string) => {
    if (listeningField) {
        setEditedSections(prev => ({
            ...prev,
            [listeningField]: (prev[listeningField] || '') + result
        }));
    }
  };
  ...
  const filteredProjects = user.projects.filter(p => p.status === activeTab).sort((a,b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());

  const parseContentToSections = (content: string): Record<string, string> => {
    const sections: Record<string, string> = {};
    const lines = content.split('\n');
    let currentTitle = 'Summary'; // Default for content before first header
  ...
  const handleEdit = (project: Project) => {
    if (user.tier === UserTier.Free) {
        alert("Please upgrade to Pro or Premium to edit projects.");
        return;
    }
  ...
  const handleSaveEdit = () => {
    ...
    const updatedProject: Project = { 
        ...editingProject, 
        content: newContent, 
        title: editedSections[''] || editingProject.title // Assuming title is the first # header
    };

    const updatedProjects = user.projects.map(p => p.id === editingProject.id ? updatedProject : p);
  ...
  const downloadPDF = (project: Project) => {
    ...
    if (user.companyLogo) {
      try {
        ...
        doc.addImage(user.companyLogo, 'JPEG', logoX, margin, logoWidth, logoHeight);
    ...
    const addText = (text: string, size: number, style: 'bold' | 'normal') => {
        ...
        lines.forEach((line: string) => {
            if (cursorY + size / 2.8 > pageHeight - margin) { // 2.8 is a good factor for mm
                doc.addPage();
                cursorY = margin;
            }
    ...
    Object.entries(sections).forEach(([title, content]) => {
        ...
        const isMainTitle = !title.match(/^\d/);
        addText(title, isMainTitle ? 18 : 14, 'bold');
        cursorY += 2;
  ...
  const renderProjectList = () => {
    if (filteredProjects.length === 0) {
      return <div className="text-center text-gray-400 p-8">No {activeTab.toLowerCase()} projects found.</div>;
    }
    return (
          ...
          <div key={project.id} className="bg-gray-800 p-4 rounded-lg shadow-md transition-transform hover:scale-[1.02]">
            <div className="flex justify-between items-start gap-4">
              ...
              <div className="flex gap-3 items-center flex-wrap justify-end shrink-0">
                {activeTab === ProjectStatus.Open && (
                  <button onClick={() => handleMarkAsCompleted(project.id)} className="text-yellow-400 hover:text-yellow-300 text-sm font-semibold">Mark Completed</button>
                )}
                <button onClick={() => handleEdit(project)} className="text-blue-400 hover:text-blue-300 text-sm disabled:text-gray-500 disabled:cursor-not-allowed" disabled={user.tier === UserTier.Free}>Edit</button>
    ...
    <div className="p-4 md:p-6 text-white h-full pt-24 pb-20 overflow-y-auto">
      ...
      <div className="flex justify-center mb-6 bg-gray-900 rounded-lg p-1 max-w-sm mx-auto">
        <button
          ...
          className={`w-1/2 py-2 rounded-md transition-colors font-semibold ${activeTab === ProjectStatus.Open ? 'bg-green-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
        >
          Open
        </button>
        <button
          onClick={() => setActiveTab(ProjectStatus.Completed)}
          className={`w-1/2 py-2 rounded-md transition-colors font-semibold ${activeTab === ProjectStatus.Completed ? 'bg-green-600 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
        >
          Completed
        </button>
      </div>
