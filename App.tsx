...
const App: React.FC = () => {
  ...
  const [currentUser, setCurrentUser] = useState<User | null>(() => {
    ...
    if (currentEmail && savedUsers) {
      ...
      const user = parsedUsers[currentEmail];
      if (user) {
        // Revive dates
      ...
      try {
        ...
        localStorage.setItem('currentUserEmail', currentUser.email);
          ...
          if (error instanceof DOMException && (error.name === 'QuotaExceededError' || error.name === 'NS_ERROR_DOM_QUOTA_REACHED')) {
              alert('Could not save changes. The application storage limit has been exceeded. Please remove some projects or reduce avatar image size.');
          } else {
  ...
  const renderView = () => {
    switch (currentView) {
      case 'home':
        return <HomeView user={currentUser} onUpdateUser={handleUpdateUser} setView={setCurrentView} />;
      case 'projects':
        return <ProjectsView user={currentUser} onUpdateUser={handleUpdateUser} />;
      case 'pricing':
        return <PricingView user={currentUser} onUpdateUser={handleUpdateUser} />;
      case 'profile':
